<script lang="ts">
	...
	const filteredRepos = data.repos.filter((e) => e.name !== 'portifolio' && e.name !== 'realfakenerd');
</script>
		...
		<ul class="grid gap-2">
			{#each filteredRepos as repo (repo.id)}
				...
				>
					...
					<div class="flex flex-row gap-2">
						<a
							class="button fill-on-primary interactive-bg-primary text-on-primary"
							rel="noreferrer"
							href={repo.html_url}
						...
						{#if repo.homepage}
							<a
								class="button fill-on-secondary interactive-bg-secondary text-on-secondary"
								rel="noreferrer"
								href={repo.homepage}
								aria-label="URL da página inicial"
								target="_blank"
							>
								<Icon
									width={18}
